...
from Sampling.sampling import sample_new_indices, SR_based_initial_set_indices, RS_initial_set_indices
...
def train_model(model_config, train_config, data_config, data_dir, logger_config,
    ...
    elif train_config['additional_params']['module'] == 'VAAL':
        AL_data_module.setup('fit')
        full_model = UNetVAAL(
            per_device_batch_size=train_config["batch_size"],
            num_devices=1,
            lr=train_config["lr"],
            ...
            val_plot_slice_interval=train_config["val_plot_slice_interval"],
            seed=seed,
            labeled_dataloader = AL_data_module.labeled_dataloader(),
            unlabeled_dataloader = AL_data_module.unlabeled_dataloader(),
            devices = gpu_devices,
            **train_config['additional_params']
        )

    early_stopping = EarlyStopping(
        monitor='val/loss',  # This is the metric to monitor
        min_delta=0.0,  # Minimum change to qualify as an improvement
        patience=train_config['patience'],  # Number of epochs with no improvement after which training will stop
        verbose=True,  # If True, prints a message when early stopping is triggered
        mode='min',  # 'min' means we stop when the metric stops decreasing
        check_on_train_epoch_end=False  # Whether to check for stopping after every training epoch
    )
    ...
    trainer = pl.Trainer(
            ...
            callbacks=[ModelCheckpoint(dirpath=os.path.join(checkpoint_path, '{}labeled'.format(len(train_config["train_indices"]))),
                        monitor='val/loss', mode='min', every_n_epochs=1, save_last=True, filename="best_model_epoch={epoch}"), early_stopping],
            logger=logger,
    ...
    metric_dict = trainer.test(full_model, datamodule=AL_data_module, ckpt_path='best')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_config',  type=str, default='data_config/data_config_mgd1k.yaml', help='Dataset for this experiment')
    parser.add_argument('--model_config', type=str, default='model_config.yaml', help='network architecture parameter configuration')
    parser.add_argument('--train_config', type=str, default='train_config.yaml', help='please note: select the appropriate configuration file.')  # LearningLoss_SB--train_learningloss_config.yaml VAAL--train_vaal_config.yaml others--train_config.yaml
    parser.add_argument('--sampling_config', type=str, default='sampling_config/Entropy.yaml', help='select one sampling strategy configuration file') # sampling strategy
    parser.add_argument('--logger_config', type=str, default='logger_config.yaml')

    # seed select
    parser.add_argument('--seed', default=0, type=int) # 0 1 42 66 88
    parser.add_argument('--num_gpu', default=1, help='number of GPU devices to use')
    parser.add_argument('--gpu_idx', default=0, type=int, help='otherwise, gpu index, if we want to use a specific gpu')
    parser.add_argument('--init_budget', default=20, type=int, help='initial labeling budget')
    # Parse the arguments first to get the seed
    args = parser.parse_args()
    # Dynamically calculate the default value for train__train_indices
    train_indices_default = list(range(args.seed, args.init_budget * 5 + args.seed, 5))

    # Training hyper-parameters that we should change according to the dataset
    parser.add_argument('--model__out_channels', default=2, type=int, help='number of output channels')
    parser.add_argument('--train__train_indices', default=None, type=int, nargs='+', help='indices of training data for Segmentation task')
    parser.add_argument('--initial_set_construction_criteria', type=str, default='SR-ILS', help='initial annotation set selection strategy: RS or SR-ILS')
    parser.add_argument('--train__loss__normalize_fct', type=str, default='sigmoid')
    parser.add_argument('--train__loss__n_classes', type=int, default=2)
    parser.add_argument('--train__val_plot_slice_interval', default=5, type=int, help='interval between 2 slices in a volume to be plotted and saved during validation')

    parser = pl.Trainer.add_argparse_args(parser)
    args = parser.parse_args()
    # We use specular reflection to fix the initial annotation set
    if args.train__train_indices is None:
        dataset_name = 'mg-203' if 'mg-203' in args.data_config else 'MGD-1K'
        if args.initial_set_construction_criteria == 'RS':
            args.train__train_indices = RS_initial_set_indices(args.data_dir, dataset_name, args.init_budget, args.seed, step=10)
        elif args.initial_set_construction_criteria == 'SR-ILS':
            args.train__train_indices = SR_based_initial_set_indices(args.data_dir, dataset_name, args.init_budget)
    ...
    for al_cycle in range(num_al_cycles):                
        ...
        new_indices, sampling_time, new_indices_uncertainty_values = sample_new_indices(sampling_config,
                                                                                        budget=train_config["sampling"]["budget"],
                                                                                        unlabeled_dataloader=AL_data_module.unlabeled_dataloader(),
                                                                                        labeled_dataloader = AL_data_module.labeled_dataloader(),
                                                                                        test_dataloader=AL_data_module.test_dataloader(),
                                                                                        trainer=trainer,
                                                                                        budget_result_path = os.path.join(checkpoint_path, '{}labeled'.format(len(train_config["train_indices"]))),
                                                                                        data_dir = args.data_dir,
                                                                                        dataset_name = data_config["dataset_name"],
                                                                                        al_cycle = al_cycle, **kwargs)
        # Transition to Stage 2
        if new_indices is None:
            new_indices, sampling_time, new_indices_uncertainty_values = sample_new_indices(sampling_config, budget=train_config["sampling"]["budget"],
                                                                                            unlabeled_dataloader=AL_data_module.unlabeled_dataloader(),
                                                                                            labeled_dataloader=AL_data_module.labeled_dataloader(),
                                                                                            test_dataloader = AL_data_module.test_dataloader(),
                                                                                            trainer=trainer,budget_result_path=os.path.join(checkpoint_path,'{}labeled'.format(len(train_config["train_indices"]))),
                                                                                            data_dir=args.data_dir,
                                                                                            dataset_name=data_config["dataset_name"],
                                                                                            al_cycle=al_cycle, **kwargs)
        ...
        time_dict = {
            ...
            'uncertainty_values': new_indices_uncertainty_values,
            'train_version': trainer.logger.version  # version control
            }
        results_path = os.path.join(checkpoint_path, '{}labeled'.format(len(train_config["train_indices"])), 'auxiliary.json')
        with open(results_path, 'w') as file:
