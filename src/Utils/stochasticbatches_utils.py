...
def select_top_positions_with_highest_diversity(top_uncertainty_indices: List[List[int]], embedding_unlabeled, embedding_labeled,  num_groups: int) -> List[List[int]]:
    aggregated_similarity_list = []
    for uncertainty_indices in top_uncertainty_indices:
        cur_embedding_unlabeled = embedding_unlabeled[uncertainty_indices]
        # Calculate intra-group similarity using cos
        cos_sim_matrix = cosine_similarity(cur_embedding_unlabeled, cur_embedding_unlabeled)
        ...
        cos_sim_matrix = cosine_similarity(cur_embedding_unlabeled, embedding_labeled)
        max_exter_similarity = np.amax(cos_sim_matrix, axis=1)
        total_max_similarity = max_intra_similarity+ max_exter_similarity
        aggregated_similarity = np.mean(total_max_similarity)
        aggregated_similarity_list.append(aggregated_similarity)
    sorted_diversity_indices = np.argsort(aggregated_similarity_list)  # Sort in ascending order
    top_groups = [top_uncertainty_indices[i] for i in sorted_diversity_indices[:num_groups]]
    return top_groups

def select_top_diversity(top_uncertainty_indices, embedding_unlabeled, embedding_labeled,  num_groups: int):
    embedding_unlabeled = embedding_unlabeled[top_uncertainty_indices]
    # Calculate intra-group similarity using cos
    cos_sim_matrix = cosine_similarity(embedding_unlabeled, embedding_unlabeled)
    np.fill_diagonal(cos_sim_matrix, -np.inf)
    max_intra_similarity = np.amax(cos_sim_matrix, axis=1)
    # Calculate the similarity with the labeled group
    cos_sim_matrix = cosine_similarity(embedding_unlabeled, embedding_labeled)
    max_exter_similarity = np.amax(cos_sim_matrix, axis=1)
    total_max_similarity = max_intra_similarity + max_exter_similarity
    sorted_diversity_indices = np.argsort(total_max_similarity)  # Sort in ascending order
    top_groups = [top_uncertainty_indices[i] for i in sorted_diversity_indices[:num_groups]]
    return top_groups

def calculate_exter_similarity(top_uncertainty_indices, embedding_unlabeled, embedding_labeled, alpha = 1.0):
    # embedding_combined = np.concatenate((embedding_unlabeled, embedding_labeled), axis=0)
    # total_cos_sim_matrix = cosine_similarity(embedding_combined, embedding_combined)
    # min_sim = np.min(total_cos_sim_matrix)
    # max_sim = np.max(total_cos_sim_matrix)
    ...
    # cos_sim_matrix = (cos_sim_matrix - min_sim) / (max_sim - min_sim)
    max_exter_similarity = np.amax(cos_sim_matrix, axis=1)
    # adaptive thresholding
    mean_similarity = np.mean(max_exter_similarity)
    std_similarity = np.std(max_exter_similarity)
    adaptive_threshold = mean_similarity + alpha * std_similarity

    return mean_similarity, std_similarity, adaptive_threshold
