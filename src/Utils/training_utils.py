...
def accuracy_ignore_background(onehot_pred, onehot_target, target_class=1):
    """
    Calculate accuracy for a specific class while ignoring background (class 0).

    Args:
        onehot_pred (Tensor): Model predictions in one-hot format [batch_size, num_classes, H, W]
        onehot_target (Tensor): Ground truth in one-hot format [batch_size, num_classes, H, W]
        target_class (int): Class index to evaluate (default=1 for foreground)

    Returns:
        float: Accuracy score between 0 and 1 for the specified class

    Note:
        Background class (class 0) is automatically excluded from calculation
    """
    # Validate input shapes match
    assert onehot_pred.shape == onehot_target.shape, "Prediction and target tensors must have identical dimensions"

    # Extract specified class channels
    pred_class = onehot_pred[:, target_class, :, :]  # [batch_size, H, W]
    target_class = onehot_target[:, target_class, :, :]  # [batch_size, H, W]

    # Compute correctly classified pixels (TP + TN)
    correct_pixels = torch.sum(pred_class == target_class)

    # Calculate total pixels in target class
    total_pixels = target_class.numel()

    # Return accuracy: correct_pixels / total_pixels
    return correct_pixels.float() / float(total_pixels)
