...
def load_mg_img(folder_path, filename_list, idx):
    image_path = os.path.join(folder_path, filename_list[idx])
    image = cv2.imread(image_path)
    image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
    # Convert the image shape from HxWxC to CxHxW
    image = np.transpose(image, (2, 0, 1))  # This changes the shape from (H, W, C) to (C, H, W)
    return image

def load_mg_label(folder_path, filename_list, idx):
    mask_path = os.path.join(folder_path, filename_list[idx])
